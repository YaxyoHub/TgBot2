...
from ustoz_shogird.states.state_menu import ShogirdState

shogird_router = Router()

@shogird_router.message(F.text == "Shogird kerak")
async def start_shogird(msg: Message, state: FSMContext):
    data = """Shogird topish uchun ariza berish
...
Oxirida agar hammasi to`g`ri bo`lsa, HA tugmasini bosing va arizangiz Adminga yuboriladi."""
    ...
    await state.set_state(ShogirdState.ism_familiya)

@shogird_router.message(ShogirdState.ism_familiya)
...
@shogird_router.message(ShogirdState.yosh)
...
@shogird_router.message(ShogirdState.texnologiya)
...
@shogird_router.message(ShogirdState.aloqa)
...
@shogird_router.message(ShogirdState.hudud)
...
Kerak bo`lsa, Summani kiriting?"""
    await msg.answer(data)
    await state.set_state(ShogirdState.narx)

@shogird_router.message(ShogirdState.narx)
...
Masalan, Talaba"""
    await msg.answer(data)
    await state.set_state(ShogirdState.kasb)

@shogird_router.message(ShogirdState.kasb)
...
Masalan, 9:00 - 18:00"""
    await msg.answer(data)
    await state.set_state(ShogirdState.murojat)

@shogird_router.message(ShogirdState.murojat)
...
@shogird_router.message(ShogirdState.maqsad)
async def get_all(msg: Message, state: FSMContext):
    await state.update_data(maqsad=msg.text)
    data = await state.get_data()
    await msg.answer(f"Shogird kerak:\n"
                     f"\n"
                     f"🎓 Ustoz: {data.get('ism_familiya')}\n"
...
@shogird_router.message(F.text == "Ha")
...
@shogird_router.message(F.text == "Yo'q")
